...
const CampaignSelectionModal = ({ isOpen, onClose, onSelect, influencer, selectedInfluencerIds = [] }) => {
  ...
  const fetchActiveCampaigns = async () => {
    try {
      ...
      const response = await fetch('http://localhost:5052/api/campaigns?status=active');
  ...
  const fetchInfluencerRequests = async () => {
    try {
      ...
      const response = await fetch(`http://localhost:5052/api/collaboration-requests?userType=influencer&userId=${influencer.id}&filter=received`);
      const result = await response.json();
        ...
        <div className="campaign-selection-modal__content">
          {loading ? (
            ...
            <div className="campaign-selection-modal__empty">
              ...
              <p>There are no active campaigns available. Please create a campaign first.</p>
            </div>
          ) : (
