...
import { getCampaigns, createCampaign, updateCampaign, deleteCampaign } from '../../../shared/services/backendApi';
...
const CampaignsPage = () => {
  ...
  const handleFormSubmit = async (e) => {
    ...
    try {
      ...
      console.log('✏️ Editing campaign:', editingCampaign);
      ...
      const sanitizedData = {
        ...
        minBudget: formData.minBudget && formData.minBudget !== '' ? parseFloat(formData.minBudget) : null,
        maxBudget: formData.maxBudget && formData.maxBudget !== '' ? parseFloat(formData.maxBudget) : null,
        campaignId: formData.campaignId && formData.campaignId !== '' ? formData.campaignId : null,
        offerDescription: formData.offerDescription && formData.offerDescription !== '' ? formData.offerDescription : null,
        kpi: formData.kpi && formData.kpi !== '' ? formData.kpi : null,
        restrictions: formData.restrictions && formData.restrictions !== '' ? formData.restrictions : null,
        previewUrl: formData.previewUrl && formData.previewUrl !== '' ? formData.previewUrl : null,
        creativeUrl: formData.creativeUrl && formData.creativeUrl !== '' ? formData.creativeUrl : null,
        countries: formData.countries && formData.countries !== '' ? formData.countries : null,
        ...
        paymentTerms: formData.paymentTerms && formData.paymentTerms !== '' ? formData.paymentTerms : null,
      };
      ...
      let result;
      if (editingCampaign) {
        console.log('📝 Updating campaign with ID:', editingCampaign.id);
        result = await updateCampaign(editingCampaign.id, sanitizedData);
      } else {
        console.log('➕ Creating new campaign...');
        result = await createCampaign(sanitizedData);
      }
      ...
      if (window.showToast) {
        window.showToast(
          (editingCampaign ? 'Error updating campaign: ' : 'Error creating campaign: ') + errorMessage,
          'error',
          5000
        );
      }
  ...
  const fetchCampaigns = async () => {
    try {
      ...
      const result = await getCampaigns({
        ...
        brandName: user?.brandName, // Filter by current user's brand
        page: 1,
        limit: 50 // Get more campaigns for the dashboard
      });
      
      if (result.success) {
        setCampaigns(result.data || []);
        setTotalUniqueInfluencers(result.data.length || 0);
      } else {
  ...
  // Update brand name in form data when user data is available
  useEffect(() => {
    if (user?.brandName) {
      setFormData(prev => ({
        ...prev,
  ...
  useEffect(() => {
    fetchCampaigns();
  }, [statusFilter, searchTerm, user?.brandName]);
                ...
                <div className="form-column">
                  ...
                  <div className="form-group">
                    ...
                    >
                      ...
                      <option value="per_influencer">Per Influencer</option>
                    </select>
                  </div>
                  ...
                  {formData.budgetType === 'per_influencer' && (
                    <div className="form-group">
                      ...
                      <div className="budget-input-group">
                        ...
                        <input
                          ...
                          value={formData.budget}
                          onChange={handleInputChange}
                          placeholder="Enter budget per influencer"
                          min="0"
                          step="0.01"
                          required
                          className="budget-input"
