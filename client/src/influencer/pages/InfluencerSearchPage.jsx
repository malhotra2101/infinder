...
const InfluencerSearchPage = ({ sidebarOpen = false }) => {
  ...
  const fetchSentRequests = useCallback(async () => {
    try {
      ...
      const response = await fetch('http://localhost:5052/api/collaboration-requests?userType=influencer&userId=16&filter=sent');
  ...
  const fetchSelectedCampaigns = useCallback(async () => {
    try {
      ...
      const response = await fetch('http://localhost:5052/api/influencers/16/campaigns');
      const result = await response.json();
  ...
  const fetchCampaigns = useCallback(async (isNewSearch = false) => {
    try {
      ...
      const response = await fetch(`http://localhost:5052/api/campaigns?${params.toString()}`);
      const result = await response.json();
  ...
  const checkCooldownStatus = useCallback(async (campaignId, brandId) => {
    try {
      const response = await fetch(`http://localhost:5052/api/rejected-influencers/check?influencerId=16&brandId=${brandId}&campaignId=${campaignId}`);
      const result = await response.json();
